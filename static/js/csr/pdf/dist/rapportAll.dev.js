...
$(document).ready(function () {
  ...
  function generateMulPdf(region, province, csr, category, year) {
    ...
    doc.text('Rapport des activités des Unités Médical Mobile (UMM)', 50, 50); // SET FONT SIZE FOR THE TEXT
    ...
    doc.save("rapport des activités des unités médical mobile (umm)  - " + csr + ' - ' + year + '.pdf');
  } // call the function
  ...
  $('#downloadRapportAllPdf').click(function () {
    ...
    generateMulPdf($(this).data('region'), $(this).data('province'), $(this).data('csr'), $(this).data('category'), today.getFullYear());
  });
});