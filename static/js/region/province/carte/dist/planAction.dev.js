...
$(document).ready(function () {
  ...
  function pdf() {
    ...
    function generateMulPdf(region, province, year) {
      ...
      doc.text('Délégation du Ministère de la Santé : ' + province, 15, 72); // ADD ALL TABLES
      ...
      for (var _i = 0; _i < listTable.length; _i++) {
        ...
        if (_i != 0) {
          ...
          doc.text(_i + 1 + '- ' + listTable[_i].title, 15, 82);
          doc.autoTable({
            html: '#' + listTable[_i].id,
            startY: 85,
            theme: 'grid',
    ...
    $('#downloadPlanActionAllPdf').click(function () {
      ...
      generateMulPdf($('#data').data('region'), $(this).data('province'), today.getFullYear());
    });
  }
  ...
  var data = $('#dataProvince').data('carte'),
      ...
      // other
  max = 100,
      scale = ['#FF4646', '#FFF891', '#fcf75a', '#a9a403', '#7DFE69', '#169a01', '#0a4600'];
  ;
  ...
  $('#province-map').vectorMap({
    ...
    onRegionClick: function onRegionClick(e, code) {
      if (provinceList.includes(parseInt(code))) showModal(planAction, code);else showErrorModal();
    }
  });
