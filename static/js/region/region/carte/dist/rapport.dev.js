...
$(document).ready(function () {
  ...
  function pdf() {
    ...
    function generateMulPdf(region, province, trimestre, year) {
      ...
      doc.text('Trimester : ' + $('#data').data('trimestre'), 15, 72); // ADD ALL TABLES
      ...
      doc.save('rapport des activités des unités médical mobile (umm)  - ' + region + ' - ' + year + '.pdf');
    } // call the function
  ...
  $('#region-map').vectorMap({
    ...
    onRegionClick: function onRegionClick(e, code) {
      if (code == codeRegion) showModal(code);else showErrorModal();
    }
  });
});